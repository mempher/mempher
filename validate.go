...
func (q JobQuery) Validate() error {
	...
	for _, kind := range q.Kinds {
		if !kind.Valid() {
			return fmt.Errorf("mempher: job kind filter %q: %w", string(kind), ErrInvalidJobKind)
		}
	}
	for _, state := range q.States {
		if !state.Valid() {
			return fmt.Errorf("mempher: job state filter %q: %w", string(state), ErrInvalidJobState)
		}
	}
	if q.Limit < 0 {
		return fmt.Errorf("mempher: job query limit is %d: %w", q.Limit, ErrInvalidConfig)
	}
	return nil
}
...
func (r PurgeRequest) Validate() error {
	if r.Before.IsZero() {
		return fmt.Errorf("mempher: purge has no horizon: %w", ErrInvalidConfig)
	}
	for _, state := range r.States {
		switch state {
		case JobStateDone, JobStateDead:
		case JobStatePending, JobStateRunning:
			return fmt.Errorf(
				"mempher: purge names %s jobs, which are still owed: %w",
				state, ErrInvalidJobState)
		default:
			return fmt.Errorf("mempher: purge state %q: %w", string(state), ErrInvalidJobState)
		}
	}
	if r.Limit < 0 {
		return fmt.Errorf("mempher: purge limit is %d: %w", r.Limit, ErrInvalidConfig)
	}
	return nil
}
...
func (q ScopeQuery) Validate() error {
	...
	if q.Limit < 0 {
		return fmt.Errorf("mempher: scope query limit is %d: %w", q.Limit, ErrInvalidConfig)
	}
	return nil
}
...
func (r BackfillRequest) Validate() error {
	...
	for _, kind := range r.Kinds {
		if !kind.Valid() {
			return fmt.Errorf("mempher: backfill kind %q: %w", string(kind), ErrInvalidJobKind)
		}
	}
	switch {
	case r.AfterSeq < 0:
		return fmt.Errorf("mempher: backfill AfterSeq is %d: %w", r.AfterSeq, ErrInvalidConfig)
	case r.AfterSeq > 0 && r.Scope == "":
		return fmt.Errorf(
			"mempher: backfill AfterSeq needs a Scope, because seq is only ordered "+
				"within one: %w", ErrInvalidConfig)
	case r.BatchSize < 0:
		return fmt.Errorf("mempher: backfill batch size is %d: %w", r.BatchSize, ErrInvalidConfig)
	case r.Limit < 0:
		return fmt.Errorf("mempher: backfill limit is %d: %w", r.Limit, ErrInvalidConfig)
	}
	return nil
}
