...
func TestScopesListsThePartitionsOfL0(t *testing.T) {
	...
	got, err := store.Scopes(ctx, ops.ScopeQuery{})
	if err != nil {
...
func TestScopesPagesAndFilters(t *testing.T) {
	...
	t.Run("pages by After", func(t *testing.T) {
		first, err := store.Scopes(ctx, ops.ScopeQuery{Limit: 2})
		if err != nil {
			t.Fatalf("Scopes: %v", err)
		}
		...
		second, err := store.Scopes(ctx, ops.ScopeQuery{After: first[1].ID, Limit: 2})
		if err != nil {
			t.Fatalf("Scopes: %v", err)
		}
		...
		last, err := store.Scopes(ctx, ops.ScopeQuery{After: second[1].ID})
		if err != nil {
			t.Fatalf("Scopes: %v", err)
		}
		if len(last) != 0 {
	...
	t.Run("filters by prefix", func(t *testing.T) {
		got, err := store.Scopes(ctx, ops.ScopeQuery{Prefix: "user:"})
		if err != nil {
			t.Fatalf("Scopes: %v", err)
		}
		if len(got) != 3 {
...
func TestScopesTreatsAPrefixAsTextNotAPattern(t *testing.T) {
	...
	got, err := store.Scopes(t.Context(), ops.ScopeQuery{Prefix: "sale:50%"})
	if err != nil {
		t.Fatalf("Scopes: %v", err)
	}
...
func TestScopesRejectsBadQueries(t *testing.T) {
	...
	if _, err := store.Scopes(ctx, ops.ScopeQuery{Limit: -1}); !errors.Is(err, mempher.ErrInvalidConfig) {
		t.Errorf("negative limit err = %v, want mempher.ErrInvalidConfig", err)
	}
	long := mempher.ScopeID(strings.Repeat("x", 300))
	if _, err := store.Scopes(ctx, ops.ScopeQuery{Prefix: long}); !errors.Is(err, mempher.ErrInvalidScope) {
		t.Errorf("over-long prefix err = %v, want mempher.ErrInvalidScope", err)
	}
	if _, err := store.Scopes(ctx, ops.ScopeQuery{After: long}); !errors.Is(err, mempher.ErrInvalidScope) {
		t.Errorf("over-long cursor err = %v, want mempher.ErrInvalidScope", err)
	}
}

func ids(scopes []ops.Scope) []mempher.ScopeID {
